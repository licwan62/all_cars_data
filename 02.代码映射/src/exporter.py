...
def create_multi_artifact_batch(
    artifact_root: Path,
    slug: str,
    regions: dict[str, tuple[list[MakeMapping], list[ModelMapping]]],
    outputs: dict[str, tuple[list[str], list[dict[str, str]]]],
    run_report: dict[str, object],
    source_path: Path,
) -> Path:
    """三区域共用的不可变运行快照：输入快照、各区域映射、合并交付物、报告。"""
    artifact_root.mkdir(parents=True, exist_ok=True)
    day = datetime.now().strftime("%Y-%m-%d")
    safe_slug = re.sub(r"[^a-z0-9-]+", "-", slug.casefold()).strip("-") or "code-mapping"
    pattern = re.compile(rf"^{re.escape(day)}_(\d{{2}})_{re.escape(safe_slug)}$")
    ...
    try:
        ...
        input_snapshot.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source_path, input_snapshot)
        for region, (make_items, model_items) in regions.items():
            write_csv_atomic(temp_path / "mapping" / region.lower() / "make_mapping.csv", MAKE_FIELDS, make_rows(make_items))
            write_csv_atomic(temp_path / "mapping" / region.lower() / "model_mapping.csv", MODEL_FIELDS, model_rows(model_items))
        for name, (fields, rows) in outputs.items():
            write_csv_atomic(temp_path / "output" / name, fields, rows)
        _write_json(temp_path / "run_report.json", {
            **run_report,
            "source_path": str(source_path),
            "source_sha256": _sha256(source_path),
            "created_at": datetime.now().astimezone().isoformat(timespec="seconds"),
        })
        _write_json(temp_path / "status.json", {
            "status": "PASS",
            "regions": {r: {"make_mappings": len(m), "model_mappings": len(mo)} for r, (m, mo) in regions.items()},
            "immutable_validation": "PASS",
            "capacity_validation": "PASS",
        })
        os.replace(temp_path, final_path)
