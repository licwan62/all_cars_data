...
- `项目/changes/`：不可覆盖的历史批次快照，不作为下游默认输入；其中所有文件均须纳入 Git。

`车形分类核定`、`车型代表分析`、`尺码簇分析`、`尺码计算`、`分类结构审核`、`适配器-Trim生成`、`销量评估` 统一使用 `changes/` 保存迭代留痕。每次迭代创建 `YYYY-MM-DD_NN_short-description/` 批次目录，保留当次结果、差异、报告、验证和研究证据；已归档批次不得覆盖或删除。
