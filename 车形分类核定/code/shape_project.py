...
def matches(item: dict[str, str], row: dict[str, str]) -> bool:
    ...
    haystack = " | ".join([f"结构={row.get('结构', '')}"] + [row.get(x, "") for x in ("版本", "CAB", "BED", "参考车型", "备注")])
...
def claim(limit: int, worker: str) -> None:
    ...
    writer = csv.DictWriter(sys.stdout, fieldnames=QUEUE_FIELDS, delimiter="\t", extrasaction="ignore", lineterminator="\n"); writer.writeheader(); writer.writerows(claimed)
...
def update(args: argparse.Namespace) -> None:
    ...
    atomic_write(CACHE, CACHE_FIELDS, cache); sync_queue(); print(f"已缓存：{target['MAKE']} {target['MODEL']} -> {args.shape}")
...
def main() -> None:
    ...
    p = sub.add_parser("batch-update"); p.add_argument("--file", type=Path, required=True)
    args = parser.parse_args()
    if args.command == "init": init()
    elif args.command == "claim": claim(args.limit, args.worker)
    elif args.command == "update": update(args)
    elif args.command == "batch-update": batch_update(args.file)
    else: build()
