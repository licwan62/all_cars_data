...
SCRIPT = ROOT / "EU尺码分析" / "pandas_analysis.py"
SOURCE = ROOT / "data" / "eu" / "0916" / "source"
spec = importlib.util.spec_from_file_location("eu_size_analysis", SCRIPT)
...
class EuPipelineTest(unittest.TestCase):
    def test_base_contract(self) -> None:
        base, summary = module.build_base(SOURCE, 2026)
        self.assertEqual(len(base), 50389)
        self.assertEqual(base["DIMENSION-ID"].nunique(), len(base))
        self.assertEqual(summary["unmatched_ktypes"], 26)
    ...
    def test_dimension_library_contract(self) -> None:
        ...
        base, _ = module.build_base(SOURCE, 2026)
        library, _ = build_dimension_library(base)
        self.assertEqual(len(library), 29707)
