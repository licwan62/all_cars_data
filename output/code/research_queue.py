...
QUEUE_DIR = ROOT / "output" / "research_queue"
QUEUE_FILE = QUEUE_DIR / "queue.csv"
CHECKPOINT_FILE = QUEUE_DIR / "checkpoint.json"
FIELDS = ["queue_key", "record_id", "MAKE", "MODEL", "版本", "YEAR", "issue_type", "current_structure", "suspected_structure", "status", "worker", "suggested_structure", "suggested_category", "confidence", "source_url", "note", "updated_at"]
...
def candidate_rows() -> list[dict[str, str]]:
    ...
    for path, issue_type, current_col, suspected_col, detail_col in sources:
        for row in read_rows(path):
            if issue_type == "uncertain" and row.get("MAKE") == "Chevrolet" and row.get("MODEL") == "Suburban":
            ...
            result.append({
                "queue_key": key(row.get("record_id", ""), issue_type, detail),
                "record_id": row.get("record_id", ""), "MAKE": row.get("MAKE", ""),
                "MODEL": row.get("MODEL", ""), "版本": row.get("版本", ""), "YEAR": row.get("YEAR", ""),
                "issue_type": issue_type, "current_structure": row.get(current_col, ""),
...
def init_queue() -> None:
    ...
    source_rows = {row.get("record_id", ""): row for row in read_rows(ROOT / "车型尺寸库.csv")}
    for row in existing.values():
        source = source_rows.get(row.get("record_id", ""), {})
        for field in ("MAKE", "MODEL", "版本", "YEAR"):
            if not row.get(field):
                row[field] = source.get(field, "")
    rows = sorted(existing.values(), key=lambda r: (r.get("status", ""), r.get("issue_type", ""), r.get("record_id", "")))
    atomic_write(rows)
