...
QUEUE_FIELDS = ["queue_key", "record_id", "MAKE", "MODEL", "版本", "YEAR", "issue_type", "current_structure", "suspected_structure", "status", "worker", "suggested_structure", "suggested_category", "confidence", "source_url", "note", "updated_at"]
...
def main() -> None:
    ...
    suburban_ids = {r["record_id"] for r in t1 if r.get("MAKE") == "Chevrolet" and r.get("MODEL") == "Suburban" and r.get("原结构") == "Pickup"}
    t1 = [r for r in t1 if r["record_id"] not in suburban_ids]
    allowed_categories = {"两厢车", "跑车", "三厢车", "越野车", "皮卡"}
    for row in t1:
        source_category = source_by_id.get(row["record_id"], {}).get("分类", row.get("原分类", ""))
    ...
    t2 = [source_by_id.get(r["record_id"], r) if r["record_id"] in suburban_ids else r for r in t2]
    for row in t2:
        if row.get("分类") not in allowed_categories:
            row["分类"] = source_by_id.get(row["record_id"], {}).get("分类", row.get("分类", ""))
    write(ART / "audit_table2_corrected.csv", t2_fields, t2)

    t3_fields, t3 = read(ART / "audit_table3_uncertain.csv")
    t3 = [r for r in t3 if r["record_id"] not in suburban_ids]
    for rid in sorted(suburban_ids, key=lambda x: source_by_id[x].get("YEAR", "")):
        row = source_by_id[rid]
        t3.append({"record_id": rid, "MAKE": row["MAKE"], "MODEL": row["MODEL"], "版本": row["版本"],
    ...
    for row in queue:
        ...
        if row.get("record_id") in suburban_ids and row.get("issue_type") == "correction":
