...
class SizeMatcher:
    ...
    def __init__(
        ...
        # Optional exclusive lower bounds: “插片指数上限” pairs with “插片指数下限”.
        self.lower_columns = {
            index: spec.rule_column.removesuffix("上限") + "下限"
            for index, spec in enumerate(self.limits)
            if spec.rule_column.endswith("上限")
        }
        ...
        self.shared_pools, self.category_pool_names = self._build_shared_pools(
            rules, include_disabled_rules
        )
        self.cache: dict[tuple[object, ...], MatchResult] = {}

    def _normalize_rule_schema(self, rules: pd.DataFrame) -> pd.DataFrame:
        """Accept the legacy rule columns and the simplified 0915/0924 structures.

        0924 drops ``模式``/``版本``; a missing ``版本`` means every rule is
        version-agnostic, exactly like an empty ``CAB``.
        """
        ...
        if "版本" not in normalized.columns:
            normalized["版本"] = pd.NA
        if "尺码池" not in normalized.columns:
            normalized["尺码池"] = pd.NA
        for column in self.lower_columns.values():
            if column in normalized.columns:
                normalized[column] = _numeric(normalized[column])
        normalized["_规则序号"] = range(len(normalized))
    ...
    def _candidate_sort_columns(self) -> list[str]:
        """Rank rules by length allowance only; insert limits just gate the queue.

        ``插片指数上限`` decides whether a size may enter the matching queue
        (the upper-limit check in ``_calculate_pool``); it is not a priority.
        Among admitted sizes the smallest length upper limit wins.  Non-length
        limits only break exact length ties, then rule-file order applies via
        the stable sort.  ``档位序号`` is intentionally excluded.
        """
    ...
    def _build_shared_pools(
        self, rules: pd.DataFrame, include_disabled_rules: bool
    ) -> tuple[
        dict[tuple[str, str | None, str | None], SizePool],
        dict[tuple[str, str | None, str | None], tuple[str, ...]],
    ]:
        """Build optional cross-category pools declared by the ``尺码池`` column."""
        normalized = rules.copy()
        if not include_disabled_rules and "使用" in normalized.columns:
            normalized = normalized.loc[
                normalized["使用"].astype("string").str.strip().str.casefold().eq("y")
            ].copy()
        for column in ["内部尺码", "分类", "尺码池", "CAB", "版本"]:
            normalized[column] = normalized[column].map(_clean_text)
        if "车型白名单" in normalized.columns:
            normalized["车型白名单"] = normalized["车型白名单"].map(_clean_text)
            normalized = normalized.loc[normalized["车型白名单"].isna()].copy()
        for column in [spec.rule_column for spec in self.limits]:
            normalized[column] = _numeric(normalized[column])
        normalized = normalized.loc[
            normalized["分类"].notna() & normalized["尺码池"].notna()
        ].copy()

        shared_pools: dict[tuple[str, str | None, str | None], SizePool] = {}
        category_names: dict[tuple[str, str | None, str | None], list[str]] = {}
        grouping = ["尺码池", "CAB", "版本"]
        for key, group in normalized.groupby(grouping, dropna=False, sort=False):
            pool_name, cab, version = key
            pool_key = (
                str(pool_name),
                None if pd.isna(cab) else str(cab),
                None if pd.isna(version) else str(version),
            ...
            for spec in self.limits:
                complete &= group[spec.rule_column].notna()
            unique_rules = group.loc[complete].drop_duplicates("_规则序号")
            candidates = (
                unique_rules.sort_values(self._candidate_sort_columns(), kind="stable")
                .to_dict(orient="records")
            )
            max_length = None
            if self.length_rule_column is not None:
                lengths = unique_rules[self.length_rule_column].dropna()
                if not lengths.empty:
                    max_length = float(lengths.max())
            shared_pools[pool_key] = SizePool(candidates, max_length)

            for category in group["分类"].dropna().unique():
                category_key = (str(category), pool_key[1], pool_key[2])
                names = category_names.setdefault(category_key, [])
                if str(pool_name) not in names:
                    names.append(str(pool_name))
        return shared_pools, {key: tuple(value) for key, value in category_names.items()}
    ...
    def _shared_pool(
        self, category: str | None, cab: str | None, version: str | None
    ) -> SizePool:
        if category is None:
            return SizePool([], None)
        names = self.category_pool_names.get((category, cab, version), ())
        candidates: list[dict[str, object]] = []
        seen: set[object] = set()
        max_lengths: list[float] = []
        for name in names:
            pool = self.shared_pools.get((name, cab, version), SizePool([], None))
            for rule in pool.candidates:
                identity = rule.get("_规则序号", id(rule))
                if identity not in seen:
                    seen.add(identity)
                    candidates.append(rule)
            if pool.max_length is not None:
                max_lengths.append(pool.max_length)
        candidates.sort(
            key=lambda rule: tuple(float(rule[column]) for column in self._candidate_sort_columns())
        )
        return SizePool(candidates, max(max_lengths) if max_lengths else None)
    ...
    def _passes_lower_limits(
        self, rule: Mapping[str, object], values: Sequence[float]
    ) -> bool:
        """A size enters the queue only when each value is above its declared lower bound."""
        for index, column in self.lower_columns.items():
            lower = rule.get(column)
            if lower is not None and not pd.isna(lower) and not values[index] > float(lower):
                return False
        return True
    ...
    def _calculate_pool(self, pool: SizePool, values: Sequence[float | None]) -> MatchResult:
        ...
        # Lower-bounded sizes (e.g. 宽头老爷车 above 插片指数 120) are not even
        # diagnostic candidates for vehicles below the bound.
        queue = [rule for rule in pool.candidates if self._passes_lower_limits(rule, numeric_values)]
    ...
    def _calculate_category(
        ...
        category_result = self._calculate_pool(self._pool(category, None, None), values)
        if category_result.has_final_candidate:
            return category_result

        # A declared 尺码池 is a second-stage fallback only.  The matching
        # category always gets first choice, including insert-index priority.
        shared_specs: list[tuple[str | None, str | None]] = []
        if category is not None and cab is not None and version is not None:
            shared_specs.append((cab, version))
        if category is not None and version is not None:
            shared_specs.append((None, version))
        shared_specs.append((None, None))
        found_shared_pool = False
        shared_result: MatchResult | None = None
        for shared_cab, shared_version in shared_specs:
            pool = self._shared_pool(category, shared_cab, shared_version)
            if not pool.candidates:
                continue
            found_shared_pool = True
            shared_result = self._calculate_pool(pool, values)
            if shared_result.has_final_candidate:
                return shared_result
        return shared_result if found_shared_pool and shared_result is not None else category_result
...
def resolve_submodel_path(
    ...
    if local_path.is_file():
        return local_path
    repository_path = PROJECT_DIR / "data" / "子车系维护表.csv"
...
def calculate(
    ...
    if body_path is None:
        body_path = next(
            (input_dir / name for name in SOURCE_FILE_ALIASES["bodies"] if (input_dir / name).is_file()),
            SHAPE_OUTPUT,
        )
    if sales_path is None:
        sales_path = next(
            (input_dir / name for name in SOURCE_FILE_ALIASES["sales"] if (input_dir / name).is_file()),
            SALES_OUTPUT,
        )
    bodies = _read_csv(body_path)
    sales = _read_csv(sales_path)
    local_reference = input_dir / "参考尺寸计算.csv"
    ...
    if submodels is None and trim_source is not None:
        ...
        # 新增或改名的尺寸记录可能尚未存在于上一版全量表。已有主键保留
        # TRIM，未命中的记录留空，等待本 agent 的 data/子车系维护表补齐。
...
def promote_file(source: Path, destination: Path) -> None:
    """Atomically promote one validated artifact file to the stable output interface."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="参考尺寸计算表；默认读取上游 03.车形分类核定/output/参考尺寸计算.csv",
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        trim_source = args.trim_source or CURRENT_OUTPUT
        ...
        result, analysis = calculate(
            ...
            sales_path=args.sales_source.resolve() if args.sales_source else None,
            reference_path=args.reference_source.resolve() if args.reference_source else None,
    ...
    summary["sales_source"] = str(args.sales_source.resolve()) if args.sales_source else str(SALES_OUTPUT)
    summary["reference_source"] = str(args.reference_source.resolve()) if args.reference_source else str(REFERENCE_DATA)
